...
def config_optimizer(local_params, global_params, global_objects, model_name="er_model"):
    # 优化器配置
    optimizer_type = local_params[f"optimizer_type{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    weight_decay = local_params[f"weight_decay{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    momentum = local_params[f"momentum{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    learning_rate = local_params[f"learning_rate{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    enable_lr_schedule = local_params[f"enable_lr_schedule{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    lr_schedule_type = local_params[f"lr_schedule_type{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    lr_schedule_step = local_params[f"lr_schedule_step{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    lr_schedule_milestones = eval(local_params[f"lr_schedule_milestones{'' if (model_name == 'er_model') else ('_' + model_name)}"])
    lr_schedule_gamma = local_params[f"lr_schedule_gamma{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    enable_clip_grad = local_params[f"enable_clip_grad{'' if (model_name == 'er_model') else ('_' + model_name)}"]
    grad_clipped = local_params[f"grad_clipped{'' if (model_name == 'er_model') else ('_' + model_name)}"]

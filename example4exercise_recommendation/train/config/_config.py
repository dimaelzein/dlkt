...
def general_config(local_params, global_params, global_objects):
    ...
    # 训练策略配置
    train_based_epoch = local_params["train_based_epoch"]
    train_strategy_config = global_params["train_strategy"]
    train_strategy_type = local_params["train_strategy"]
    train_strategy_config["train_based_epoch"] = train_based_epoch
    ...
    if train_based_epoch:
        ...
        num_step = local_params["num_step"]
        num_iter = num_step
        num_step2evaluate = local_params["num_step2evaluate"]
        use_early_stop = local_params["use_early_stop"]
        num_early_stop = local_params["num_early_stop"]

        train_strategy_config["num_step"] = num_step
        train_strategy_config["num_step2evaluate"] = num_step2evaluate
        train_strategy_config["valid_test"]["use_early_stop"] = use_early_stop
        if use_early_stop:
            train_strategy_config["valid_test"]["num_early_stop"] = num_early_stop
            train_strategy_config["no_test"]["num_early_stop"] = num_early_stop
    ...
    # 配置evaluate参数
    top_ns = local_params["top_ns"]
    global_params["top_ns"] = eval(top_ns)

    global_objects["logger"].info(
        "basic setting\n"
        f"    device: {global_params['device']}, seed: {global_params['seed']}\n"
        "train policy\n"
        f"    train based on {'epoch' if train_based_epoch else 'step'}, type: {train_strategy_type}, "
        f"{train_strategy_type}: {json.dumps(train_strategy_config[train_strategy_type])}, "
        f"train batch size: {local_params['train_batch_size']}, num of {'epoch' if train_based_epoch else 'step'}: "
        f"{num_iter}\n"
        "evaluate metric\n"
        f"    main metric: {main_metric}, use multi metrics: {use_multi_metrics}, top_ns: {top_ns}"
        f"{f', multi metrics: {mutil_metrics}' if use_multi_metrics else ''}"
    )

    # 优化器配置
    global_objects["logger"].info("optimizer setting")
    config_optimizer(local_params, global_params, global_objects, model_name="er_model")

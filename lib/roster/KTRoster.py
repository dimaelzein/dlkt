...
class KTRoster:
    def __init__(self, global_params, global_objects):
        self.params = global_params
        self.objects = global_objects
        self.kt_model = self.objects["models"]["kt_model"]

    def process_batch(self, batch):
        if type(batch) is list:
            max_seq_len = max(list(map(lambda x: len(x["correct_seq"]), batch)))
    ...
    def get_last_concept_mastery_level(self, batch):
        return self.kt_model.get_last_concept_mastery_level(self.process_batch(batch))

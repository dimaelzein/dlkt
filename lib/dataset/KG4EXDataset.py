...
class KG4EXDataset(Dataset):
    ...
    def load_dataset(self):
        ...
        self.triples = read_triple(dataset_path, self.objects["data"]["entity2id"], self.objects["data"]["relation2id"])
    ...
    def __getitem__(self, idx):
        dataset_this = self.params["datasets_config"]["dataset_this"]
        is_train = dataset_this == "train"
        dataset_config_this = self.params["datasets_config"][dataset_this]
        negative_sample_size = dataset_config_this.get("negative_sample_size", None)
        mode = dataset_config_this["mode"]
        num_entity = len(self.objects["data"]["entity2id"])

        if is_train:
            positive_sample = self.triples[idx]
            head, relation, tail = positive_sample
            subsampling_weight = self.count[(head, relation)] + self.count[(tail, -relation - 1)]
            subsampling_weight = torch.sqrt(1 / torch.Tensor([subsampling_weight])).to(self.params["device"])
            ...
            negative_sample = torch.LongTensor(negative_sample).to(self.params["device"])
            positive_sample = torch.LongTensor(positive_sample).to(self.params["device"])
            ...
            tmp = torch.LongTensor(tmp).to(self.params["device"])
            filter_bias = tmp[:, 0].float()
            negative_sample = tmp[:, 1]
            positive_sample = torch.LongTensor((head, relation, tail)).to(self.params["device"])
